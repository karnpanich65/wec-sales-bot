...
try:
    ...
    _R124_EXAM = [
        ...
        ("P1", "ถามอายุซ้ำ", "_r135_is_age_ask", ("ขอถามอีกนิดนะครับ ลูกค้าอายุประมาณเท่าไหร่ครับ",), ("truthy", ""), "คำถามอายุตัวยาว ต้องจับได้ (เคส Nunny 3 ก.ย. ถาม 2 รอบ)"),
        ("P2", "ถามอายุซ้ำ", "_r135_is_age_ask", ("เดี๋ยวประเมินวงเงินคร่าวๆ ให้เลยค่ะ คุณลูกค้าอายุเท่าไหร่คะ",), ("truthy", ""), "คำถามอายุที่พ่วงมากับประโยคประเมินวงเงิน ต้องจับได้ด้วย"),
        ...
        ("Q2", "สรุปทวน", "_r136_recap", ({"data": {"income_baht": 40000, "income_total": 40000, "debt_baht": 0, "age": 29}},), ("no", "ล้าน"), "ห้ามมีตัวเลขวงเงินในแชทเลย (Gift เคาะ 3 ก.ย. 69 ให้เซลโทรคุย) — ถ้าจะเปิด _R136_SHOW_CAP กลับ ต้องมาแก้ข้อสอบข้อนี้ด้วย จะได้ไม่เปิดโดยไม่ตั้งใจ"),
        ...
        ("R1", "สรุปทวน", "_r136_recap", ({"data": {"income_baht": 40000, "income_total": 40000, "debt_baht": 0, "age": 29}},), ("no", "ประมาณการเบื้องต้น"), "Gift เคาะ 3 ก.ย. — วงเงินไม่ต้องโชว์ในแชท ให้เซลโทรคุย"),
        ("R2", "สรุปทวน", "_r136_recap", ({"data": {"income_baht": 40000, "income_total": 40000, "debt_baht": 0, "age": 29}},), ("has", "ที่ปรึกษาโทร"), "ต้องบอกลูกค้าว่าเซลจะโทรมาคุยวงเงิน ไม่ใช่เงียบไปเฉยๆ"),
        ...
        ("T1", "สรุปทวน", "_r136_recap", ({"data": {"income_total": 100000, "co_borrower_income": 100000, "age": 51}},), ("no", "รายได้ 100,000"), "income_total คือรายได้รวมกับผู้กู้ร่วม ห้ามขึ้นว่าเป็นรายได้ลูกค้า (เคสจริง 3 ก.ย. 12:26)"),
        ("T2", "สรุปทวน", "_r136_recap", ({"data": {"income_baht": 27370, "co_borrower_income": 100000, "age": 51}},), ("has", "รายได้ผู้กู้ร่วม 100,000"), "รายได้ผู้กู้ร่วมต้องแยกบรรทัด จะได้ไม่ปนกับรายได้ลูกค้า"),
        ("T3", "สรุปทวน", "_r136_own_income", ({"income_total": 100000, "income_baht": 0},), ("falsy", ""), "ไม่มีรายได้ตัวเอง ต้องคืน 0 ห้ามหยิบยอดรวมมาแทน"),
        ("T4", "แก้ข้อมูล", "_r136_is_wrong", ("-รายได้ไม่ถึง 100,000/เดือนคะ",), ("truthy", ""), "คำว่าไม่ถึง ต้องนับเป็นการแจ้งว่าข้อมูลผิด (เดิมหลุด ด่านแก้ไม่ทำงาน)"),
        ("T5", "แก้ข้อมูล", "_r136_apply_fix", (bot, "รายได้ไม่ถึง 100,000 คะ", {"data": {"income_baht": 27370}}), ("falsy", ""), "บอกทิศไม่บอกเลขจริง ห้ามบันทึกเลขที่ลูกค้าเพิ่งปฏิเสธ"),
        ("U1", "ชื่อลูกค้า", "_r137_name_from_reply", ("สองหมื่นครับ",), ("falsy", ""), "ยอดผ่อนเป็นตัวหนังสือ ห้ามกลายเป็นชื่อลูกค้า (เคสจริง 3 ก.ย. 13:16 ชีตขึ้นชื่อ 'สองหมื่น')"),
        ("U2", "ชื่อลูกค้า", "_r137_name_from_reply", ("ประมาณ8แสน ไม่รวมบ้าน",), ("falsy", ""), "ยอดหนี้ ห้ามกลายเป็นชื่อ"),
        ("U3", "ชื่อลูกค้า", "_r137_name_from_reply", ("กิ๊กครับ",), ("truthy", ""), "ชื่อจริงต้องยังเก็บได้ตามเดิม ห้ามเข้มจนเก็บชื่อไม่ได้เลย"),
        ("U4", "ชื่อลูกค้า", "_r137_name_from_reply", ("Mild",), ("truthy", ""), "ชื่อภาษาอังกฤษต้องยังผ่าน"),
        ("U5", "คำตอบข้ามคิว", "_r137_not_age", ("ไม่ถึง 2 หมื่น",), ("truthy", ""), "ตอบยอดผ่อนตอนบอทถามอายุ ต้องรู้ว่าไม่ใช่อายุ แล้วคงธงรอไว้"),
        ("U6", "คำตอบข้ามคิว", "_r137_not_age", ("27",), ("falsy", ""), "27 คืออายุ ต้องรับแล้วปิดธง"),
        ("V1", "ขอเบอร์อย่างเดียว", "_r138_reask", ("",), ("has", "ขอเบอร์"), "ข้อความถามซ้ำต้องยังขอเบอร์อยู่ (ด่านอื่นเช็คคำนี้ด้วย ห้ามหาย)"),
        ("V2", "ขอเบอร์อย่างเดียว", "_r138_reask", ("",), ("no", "ID LINE"), "ห้ามเสนอไลน์เอง (Gift 3 ก.ย. 69 เน้นขอเบอร์ · กติกาเดิม 17 ส.ค. 69)"),
        ("V3", "ขอเบอร์อย่างเดียว", "_r138_reask_female", ("",), ("no", "ครับ"), "เสียงหญิงต้องแก้คู่กันเสมอ (บทเรียน r45 แก้ฝั่งเดียวแล้วอีกฝั่งตกยุค)"),
        ("V4", "ขอเบอร์อย่างเดียว", "_r138_reask_female", ("",), ("has", "ขอเบอร์"), "เสียงหญิงต้องขอเบอร์เหมือนกัน"),
        ("V5", "ขอเบอร์อย่างเดียว", "_r138_prompt_clean", ("",), ("truthy", ""), "prompt ของ AI ต้องไม่เหลือคำสั่งให้ขอไลน์ ไม่งั้น AI แต่งเสนอไลน์เอง"),
        ("W1", "หนี้ปนรายได้", "_r139_exam_pick", ("",), ("has", "35,000"), "ต้องหยิบเงินเดือน 35,000 ไม่ใช่ยอดหนี้ (เคสจริง FB-RS-20260903-516)"),
        ("W2", "หนี้ปนรายได้", "_r139_exam_pick", ("",), ("no", "1739"), "ยอดหนี้ 1.7 ล้าน ห้ามกลายเป็นเงินเดือน (ของเดิมอ่านได้ 1,173,900 -> วงเงิน 142 ล้าน)"),
        ("W3", "หนี้ปนรายได้", "_r139_income_clause", ("พนักงานประจำ รายได้ 35,000",), ("has", "35,000"), "ข้อความปกติที่ไม่มีคำว่าหนี้ ห้ามไปตัดอะไรทิ้ง"),
        ("W4", "หนี้ปนรายได้", "_r139_income_clause", ("เงินเดือน 40000",), ("eq", "เงินเดือน 40000"), "ไม่เข้าเงื่อนไข ต้องคืนข้อความเดิมเป๊ะ"),
        ("W5", "สัญญาณเสี่ยง", "_r139_risk", ("ยังไม่ผิดนัดชำระ แต่เริ่มจ่ายไม่ไหว",), ("truthy", ""), "ลูกค้าบอกเองว่าจ่ายไม่ไหว ต้องติดธงแดงให้เซลเห็น"),
        ("W6", "สัญญาณเสี่ยง", "_r139_risk", ("ผ่อนรถอยู่ 8000 สบายๆ ครับ",), ("falsy", ""), "ผ่อนปกติ ห้ามติดธงแดงมั่ว"),
        ("X1", "ช่วงรายได้คำไทย", "_r140_exam_range", ("",), ("eq", "40000-80000"), "4หมื่นถึง7-8 หมื่น ต้องอ่านเป็นช่วง 40,000-80,000 (เคสจริง FB-MA-20260903-771 ของเดิมอ่านไม่ออก ไม่มีธงเตือน)"),
        ("X2", "ช่วงรายได้คำไทย", "_r140_income_range", ("เงินเดือน 40000",), ("falsy", ""), "ไม่ใช่ช่วง ห้ามตีเป็นช่วง"),
        ("X3", "ช่วงรายได้คำไทย", "_r140_income_range", ("รายได้ 40000-80000",), ("truthy", ""), "แบบตัวเลขอารบิกเดิม ต้องยังทำงานเหมือนเดิม"),
...
# ======================================================
# r135 — ถามอายุแล้วต้องมีคนรับคำตอบ (เคส Nunny Chanunto 3 ก.ย. 2026 10:43)
# ------------------------------------------------------
#   บอท "เดี๋ยวประเมินวงเงินคร่าวๆ ให้เลยค่ะ คุณลูกค้าอายุเท่าไหร่คะ"
#   ลูกค้า "39 ค่ะ"
#   บอท "ขอถามอีกนิดนะคะ ลูกค้าอายุประมาณเท่าไหร่คะ ..."   <- ถามซ้ำ
#
# ต้นเหตุ: ในระบบมีคำถามอายุ 2 ตัว
#   1) AGE_Q (faq_data) — ตั้งธง state[awaiting_age] ถูกต้อง
#   2) คำถามอายุที่พ่วงมากับประโยคประเมินวงเงิน — ไม่ได้ตั้งธงเลย
#   bot_logic บรรทัด ~2707 รับคำตอบเฉพาะตอนธง awaiting_age ขึ้น
#   -> ตัวที่ 2 ถามแล้วไม่มีใครรับ -> data[age] ยังว่าง
#   -> ด่านบรรทัด ~2907 เห็นว่า age ยังว่าง -> ยิง AGE_Q ซ้ำอีกรอบ
#
# แก้ที่ต้นเหตุ: เทิร์นไหนก็ตามที่มีบับเบิลถามอายุ -> ตั้งธงรอคำตอบเสมอ
#   พอเก็บอายุได้ ด่าน ~2907 จะไม่ยิง AGE_Q เอง = ไม่ต้องไปตัดบับเบิลทีหลัง
# กันชนอีกชั้น: ถ้า data[age] มีค่าแล้ว ยังมีบับเบิลถามอายุโผล่มา -> ตัดทิ้ง
#   (ถ้าตัดแล้วไม่เหลืออะไรเลย = ไม่ตัด กติกา ห้ามทำให้บอทเงียบ)
#
# ครอบคลุมกว่า r132 ข้อ (ข) ที่เทียบคำตรงๆ — "อายุประมาณเท่าไหร่" หลุดตัวกรองเดิม
#
# ผลข้างเคียงที่ตั้งใจ: เคสอายุ 50+ ที่ถามผ่านคำถามตัวที่ 2 จะเริ่มเข้า
#   เส้นทางเสนอผู้กู้ร่วม (ตรรกะเดิมของ bot_logic ~2713 ที่มีอยู่แล้ว
#   แต่ก่อนหน้านี้ไม่เคยทำงานจากคำถามตัวนี้ เพราะอายุไม่เคยถูกเก็บ)
# ======================================================
_R135_AGE_RE = re.compile("อายุ.{0,15}?(เท่าไหร่|เท่าไร|เท่าไหร|กี่ปี)")


def _r135_is_age_ask(text):
    """บับเบิลนี้เป็นคำถามอายุหรือเปล่า"""
    return bool(_R135_AGE_RE.search(str(text or "")))


def _r135_age_val(state):
    d = (state or {}).get("data") or {}
    try:
        return int(d.get("age") or 0)
    except Exception:
        return 0


_R135_BASE_DECIDE = CalmBotEngine._decide


def _decide_r135(self, msg, user_id, state, bucket, is_new):
    out = _R135_BASE_DECIDE(self, msg, user_id, state, bucket, is_new)
    try:
        bubbles, grade = out
        if bubbles:
            hit = [i for i, b in enumerate(bubbles) if _r135_is_age_ask(b)]
            if hit:
                _have = _r135_age_val(state)
                if _have > 0:
                    keep = [b for i, b in enumerate(bubbles)
                            if i not in set(hit)]
                    if keep:
                        print(f"[R135] มีอายุ {_have} ในระบบแล้ว — ตัดคำถามอายุซ้ำ")
                        return keep, grade
                elif not state.get("awaiting_age"):
                    state["awaiting_age"] = True
                    print("[R135] เทิร์นนี้ถามอายุ — ตั้งธงรอคำตอบ "
                          "(เดิมถามแล้วไม่มีใครรับคำตอบ)")
    except Exception as e:
        print(f"[R135 ERROR] {e}")
    return out
...
# Gift 3 ก.ย. 2026: "วงเงินไม่ต้อง ให้เซลโทรคุย"
# เหตุผลจากหน้างาน: ตัวเลขวงเงินในแชทกลายเป็นความคาดหวังทันที
# พอธนาคารให้ไม่เท่านั้น เซลเป็นคนรับหน้าแทน
# และลูกค้าที่ได้ตัวเลขไปแล้วมักหายไปเลย ไม่รอสาย
# ปิดสวิตช์ไว้ที่นี่ เปิดกลับได้ทันที (ตัวคิดวงเงินยังอยู่ครบ ใช้ตีเกรดตามเดิม)
_R136_SHOW_CAP = False
_R136_SKIP = ("contact_refused", "below_threshold", "soft_close",
              "owner", "renter", "bot_off", "handover", "closed")
_R136_WRONG = ("ไม่ใช่", "ไม่ตรง", "ไม่ถูก", "ผิดครับ", "ผิดค่ะ", "ไม่ถูกต้อง",
               ...
               # เคสจริง 3 ก.ย. 12:26 ลูกค้าตอบ "รายได้ไม่ถึง 100,000/เดือนคะ"
               # ลิสต์เดิมไม่มีคำว่า "ไม่ถึง" -> ด่านแก้ข้อมูลไม่ทำงานเลย
               "ไม่ถึง", "ไม่เกิน", "น้อยกว่า", "มากกว่า", "ต่ำกว่า", "สูงกว่า",
               "ที่จริง", "จริงๆ", "อัปเดต", "อัพเดต")

# บอกทิศแต่ไม่บอกตัวเลขจริง ("ไม่ถึง 100,000") = ห้ามเอาเลขนั้นไปเป็นค่าใหม่
# ต้องถามกลับให้ได้ตัวเลขจริงก่อน ไม่งั้นจะบันทึกเลขที่ลูกค้าเพิ่งปฏิเสธ
_R136_VAGUE = ("ไม่ถึง", "ไม่เกิน", "น้อยกว่า", "มากกว่า", "ต่ำกว่า", "สูงกว่า")
...
def _r136_own_income(d):
    """รายได้ของ "ตัวลูกค้าเอง" เท่านั้น ห้ามใช้ income_total

    เคสจริง 3 ก.ย. 12:26 (Leads_MillionCondo แถว 265)
      สัญญาณในแถวเขียนไว้ว่า "กู้ร่วม: 0 + 100,000 = รายได้รวม 100,000"
      income_total = ตัวเอง + ผู้กู้ร่วม ไม่ใช่เงินเดือนลูกค้า
      แต่สรุปทวนหยิบ income_total มาขึ้นว่า "รายได้ 100,000 / เดือน"
      ลูกค้าตอบกลับทันที "รายได้ไม่ถึง 100,000/เดือนคะ"
    -> สรุปต้องใช้รายได้ตัวเองเท่านั้น ผู้กู้ร่วมแยกอีกบรรทัด
    """
    n = _r136_int(d.get("income_baht"))
    if n:
        return n
    try:
        return _r136_int(_bl131._parse_income(str(d.get("income") or "")))
    except Exception:
        return 0
...
def _r136_lines(state):
    ...
    d = (state or {}).get("data") or {}
    out = []
    inc = _r136_own_income(d)
    if inc:
        out.append("• รายได้ " + format(inc, ",") + " / เดือน")
    _co = _r136_int(d.get("co_borrower_income"))
    if _co:
        out.append("• รายได้ผู้กู้ร่วม " + format(_co, ",") + " / เดือน")
    dm = d.get("debt_baht")
    if dm is None:
...
def _r136_cap_line(state):
    ...
    d = (state or {}).get("data") or {}
    inc = _r136_own_income(d) + _r136_int(d.get("co_borrower_income"))
    if not inc:
        return ""
...
def _r136_recap(state):
    ...
    """
    _d = (state or {}).get("data") or {}
    _real = ((1 if _r136_own_income(_d) else 0)
             + (1 if _r136_int(_d.get("co_borrower_income")) else 0)
    ...
    if _R136_SHOW_CAP:
        ...
        if cap:
            parts.append(cap)
    parts.append("")
    parts.append("ถ้าตรงแล้วไม่ต้องตอบก็ได้ครับ "
                 "เดี๋ยวที่ปรึกษาโทรไปคุยเรื่องวงเงินให้ละเอียดครับ")
    _eg = ("ผ่อนบ้าน 8,000"
...
def _r136_apply_fix(self, msg, state):
    ...
    if _bl131._has_any(s, _R136_VAGUE):
        # "ไม่ถึง 100,000" = ปฏิเสธเลขนั้น ไม่ใช่ให้เลขใหม่
        # ถ้าเก็บ = บันทึกเลขที่ลูกค้าเพิ่งบอกว่าไม่ใช่ -> ปล่อยไปถามกลับแทน
        print("[R136] ลูกค้าบอกทิศแต่ไม่ให้ตัวเลขจริง — ไม่แก้ ให้ไปถามกลับ")
        return done
    if _bl131._has_any(s, _bl131._INCOME_SAYS):
        ...
        if n:
            ...
            # ลดรายได้ = อันตรายที่สุดในบล็อกนี้ เกรดร่วง เคสหลุดคิวแจก
            # เคสที่กลัว: "มีรายได้เสริมอีก 10,000 ครับ" -> ห้ามทับ 25,000 เป็น 10,000
            # ยอมลดเฉพาะตอนลูกค้าบอกชัดว่าแก้ หรือกำลังตอบคำถามที่บอทถามกลับ
            if (old_b and int(n) < old_b
                    and not _r136_is_wrong(s)
                    and not state.get("r136_asked")):
                self._add_signal(
                    state,
                    "⚠️ ลูกค้าพูดถึงรายได้ " + format(int(n), ",")
                    + " หลังทวน (ต่ำกว่าที่บันทึกไว้ " + format(old_b, ",")
                    + ") — ไม่แก้อัตโนมัติ ให้เซลถามตอนโทร (r136)")
                print("[R136] ตัวเลขรายได้ต่ำกว่าเดิม ไม่มีคำแก้ชัด — ไม่ทับ ติดธงแทน")
            else:
                self._capture(state, "income", s)
                d["income_baht"] = int(n)
                if old_t:
                    # income_total อาจรวมรายได้ผู้กู้ร่วม -> ขยับเฉพาะส่วนของลูกค้า
...
# ======================================================
# r137 — คำตอบมาผิดคิว: กันชื่อเพี้ยน + คงธงรออายุไว้ให้ตอบทีหลังได้
# ------------------------------------------------------
# (ก) เคสจริง Saknarin Kruerta 3 ก.ย. 13:16 (LINE ที่ทีมมาร์เก็ตติ้งส่งมา)
#     บอทถาม "ขอชื่อที่ให้ที่ปรึกษาเรียกด้วยค่ะ" พร้อมถามยอดหนี้ในเทิร์นเดียว
#     ลูกค้าตอบยอดหนี้ ธงรอชื่อยังค้าง -> เซล (คน) ถาม "ผ่อนต่อเดือนเท่าไหร่"
#     ลูกค้าตอบ "สองหมื่นครับ" -> ธงรอชื่อกินคำตอบนั้น
#     -> ชีตขึ้นชื่อลูกค้าว่า "สองหมื่น" (เห็นในภาพ Dispatch_Log FB-MA-20260903-774)
#     ต้นเหตุ: _looks_like_name กันแค่ "มีตัวเลขอารบิก" แต่ "สองหมื่น" เป็นตัวหนังสือล้วน
#     กติกา Gift ในโค้ดเดิม: "เก็บชื่อผิดแย่กว่าไม่เก็บ" -> เข้มเพิ่มได้
#
# (ข) เคสจริง Forget Praewa 3 ก.ย. 10:0x (Chat_Log_WealthOwner 5657-5661)
#     5657 บอทถามอายุ -> r135 ตั้งธงรออายุ
#     5658 ลูกค้าตอบ "ไม่ถึง 2 หมื่น"  <- ตอบยอดผ่อน ไม่ใช่อายุ
#          ธงรออายุถูกกินทิ้งไปเฉยๆ (bot_logic ~2707 pop ทันทีไม่ว่าตอบอะไร)
#     5660 ลูกค้าตอบ "27"             <- นี่คืออายุ มาช้า 2 เทิร์น
#          ไม่มีใครรับแล้ว -> อายุหาย -> เคสปิดเป็น done ทั้งที่ไม่รู้อายุ
#     แก้: ถ้าคำตอบอ่านเป็นอายุไม่ได้ ให้คงธงไว้อีกรอบ (โควตา 2 รอบ)
#          ลูกค้าตอบข้ามคิวได้ ไม่ต้องบังคับให้ตอบเรียงลำดับ
# ======================================================
_R137_NOT_NAME = ("หมื่น", "แสน", "ล้าน", "พัน", "ร้อย", "บาท", "เดือน", "ปี",
                  "ผ่อน", "หนี้", "รายได้", "เงินเดือน", "ไม่มี", "ไม่ทราบ",
                  "ประมาณ", "เท่าไหร่", "เท่าไร", "หนึ่ง", "สอง", "สาม", "สี่",
                  "ห้า", "หก", "เจ็ด", "แปด", "เก้า", "สิบ")

_R137_BASE_NAME = _bl131._name_from_reply


def _r137_name_from_reply(msg):
    """ด่านหน้าของ _name_from_reply — จำนวนเงินห้ามกลายเป็นชื่อลูกค้า"""
    s = str(msg or "")
    if _bl131._has_any(s, _R137_NOT_NAME):
        print("[R137] ไม่เก็บเป็นชื่อ (มีคำบอกจำนวน): " + s[:40])
        return ""
    try:
        if _bl131._parse_debt_monthly(s) or _bl131._parse_income(s):
            print("[R137] ไม่เก็บเป็นชื่อ (อ่านออกเป็นจำนวนเงิน): " + s[:40])
            return ""
    except Exception:
        pass
    return _R137_BASE_NAME(msg)


_bl131._name_from_reply = _r137_name_from_reply


def _r137_not_age(msg):
    """ข้อความนี้อ่านเป็นอายุไม่ได้ใช่ไหม (ใช้ตัดสินว่าจะคงธงรออายุไว้ไหม)"""
    try:
        return _bl131._parse_age(str(msg or "")) is None
    except Exception:
        return False


_R137_AGE_ROUNDS = 2
_R137_BASE_DECIDE = CalmBotEngine._decide


def _decide_r137(self, msg, user_id, state, bucket, is_new):
    _keep = False
    try:
        _keep = bool(state.get("awaiting_age")) and _r137_not_age(msg)
    except Exception as e:
        print(f"[R137 ERROR pre] {e}")

    out = _R137_BASE_DECIDE(self, msg, user_id, state, bucket, is_new)

    try:
        if _keep and not state.get("done") and not state.get("awaiting_age"):
            _d = state.get("data") or {}
            _have = 0
            try:
                _have = int(_d.get("age") or 0)
            except Exception:
                _have = 0
            _n = int(state.get("r137_age_wait") or 0)
            if not _have and _n < _R137_AGE_ROUNDS:
                state["r137_age_wait"] = _n + 1
                state["awaiting_age"] = True
                print(f"[R137] คำตอบไม่ใช่อายุ — คงธงรออายุไว้ (รอบ {_n + 1}/{_R137_AGE_ROUNDS})")
    except Exception as e:
        print(f"[R137 ERROR post] {e}")
    return out


CalmBotEngine._decide = _decide_r137


# ======================================================
# r138 — ขอเบอร์อย่างเดียว เลิกเสนอไลน์เอง (Gift 3 ก.ย. 2026 "เน้นขอเบอร์ไง")
# ------------------------------------------------------
# ทีมมาร์เก็ตติ้งแจ้ง 3 ก.ย. 16:53 "มันกลับมาขอไอดีไลน์อีกแล้วค่ะ"
# เคส Forget Praewa เห็นชัด บอทเสนอไลน์เอง 2 บับเบิลจาก 4
#   "ถ้าไม่สะดวกให้เบอร์ ขอเป็น ID LINE ก็ได้ค่ะ ..."           <- CONTACT_REASK_MSG
#   "ได้ค่ะ ถ้าไม่สะดวกคุยโทรศัพท์ ขอเป็น ID LINE ก็ได้ค่ะ ..."  <- AI แต่งเองตาม prompt
#
# ขัดกับกติกาที่ Gift เคาะไว้เองตั้งแต่ 17 ส.ค. 2026 (คอมเมนต์ใน QUALIFY_QUESTIONS[3])
#   "ขอเบอร์อย่างเดียว ไม่เสนอ LINE
#    โทรคุยปิดเคสง่ายกว่าแชทมาก แชทยืดแล้วเงียบหาย
#    (ยังรับ LINE ได้ถ้าลูกค้าให้มาเอง แค่ไม่เป็นฝ่ายเสนอ)"
# ข้อความ 2 ชุดนี้ตกยุคไป เลยกลายเป็นบอทเปิดประตูไลน์ให้เอง
#
# r138 แก้ 2 ชั้น
#   1 ข้อความถามซ้ำ (ตายตัว) - เอาไลน์ออก เหลือขอเบอร์ + ถามช่วงเวลาที่สะดวก
#     ทำทั้งเสียงชายและเสียงหญิง (บทเรียน r45 - แก้ฝั่งเดียวแล้วอีกฝั่งตกยุค)
#   2 prompt ของ AI - ตัดคำสั่งที่บอกให้ขอ "เบอร์/ID LINE" ออก
#     แล้วเติมกฎห้ามเสนอไลน์ก่อน
#
# ไม่แตะด่านรับข้อมูล - ลูกค้าพิมพ์ไอดีไลน์มาเองยังเก็บได้เหมือนเดิมทุกประการ
# ไม่ใส่โควตาคำขอเบอร์ - Gift สั่งว่าเบอร์คือของสำคัญ ขอไปจนกว่าจะได้
# ======================================================
_R138_REASK_M = ("ขอเบอร์ติดต่อกลับอีกครั้งนะครับ "
                 "ที่ปรึกษาโทรอธิบายจบในสายเดียว ลูกค้าไม่ต้องพิมพ์ตอบยาว "
                 "สะดวกให้โทรช่วงไหนครับ")
_R138_REASK_F = ("ขอเบอร์ติดต่อกลับอีกครั้งนะคะ "
                 "ที่ปรึกษาโทรอธิบายจบในสายเดียว ลูกค้าไม่ต้องพิมพ์ตอบยาว "
                 "สะดวกให้โทรช่วงไหนคะ")

_bl131.CONTACT_REASK_MSG = _R138_REASK_M
try:
    _bl131._FEMALE_EXACT[_R138_REASK_M] = _R138_REASK_F
except Exception as e:
    print(f"[R138 ERROR female] {e}")

_R138_OLD_PROMPT = "ขอเบอร์ติดต่อ/ID LINE"
_R138_NEW_PROMPT = "ขอเบอร์ติดต่อ"
_R138_RULE_ANCHOR = "กฎที่ต้องปฏิบัติเสมอ"
_R138_RULE_ADD = ("\n- ขอ \"เบอร์โทร\" อย่างเดียว ห้ามเสนอ ID LINE เป็นทางเลือกเอง "
                  "(ถ้าลูกค้าพิมพ์ไอดีไลน์มาเอง รับได้ตามปกติ) "
                  "โทรคุยปิดเคสง่ายกว่าแชท - Gift 17 ส.ค. 2026")


def _r138_scrub_prompts():
    """ไล่แก้ prompt ทุกก้อนที่ยังสั่งให้ขอไลน์ + เติมกฎห้ามเสนอไลน์

    ไล่จากเนื้อความ ไม่ผูกกับชื่อตัวแปร - ย้ายที่อยู่แล้วยังทำงาน
    """
    import faq_data as _fq138
    hits = []
    for _mod in (_fq138, _bl131):
        for _n in dir(_mod):
            if _n.startswith("__"):
                continue
            try:
                ...
                continue
            if not isinstance(_v, str) or len(_v) < 40:
                continue
            _new = _v
            if _R138_OLD_PROMPT in _new:
                _new = _new.replace(_R138_OLD_PROMPT, _R138_NEW_PROMPT)
            if _R138_RULE_ANCHOR in _new and "ห้ามเสนอ ID LINE" not in _new:
                _new = _new.replace(_R138_RULE_ANCHOR,
                                    _R138_RULE_ADD + "\n\n" + _R138_RULE_ANCHOR, 1)
            if _new != _v:
                try:
                    setattr(_mod, _n, _new)
                    hits.append(_mod.__name__ + "." + _n)
                except Exception:
                    pass
    return hits
...
try:
    _R138_HITS = _r138_scrub_prompts()
    print("[R138] ขอเบอร์อย่างเดียว - แก้ prompt " + str(len(_R138_HITS)) + " ก้อน "
          + (", ".join(_R138_HITS[:4]) if _R138_HITS else "(ไม่พบก้อนที่ต้องแก้)"))
except Exception as e:
    _R138_HITS = []
    print(f"[R138 ERROR scrub] {e}")


def _r138_reask(_=None):
    """ข้อความขอเบอร์ซ้ำที่ใช้จริงตอนนี้ (อ่านสด ไม่ใช่ค่าที่จำไว้)"""
    return str(getattr(_bl131, "CONTACT_REASK_MSG", ""))


def _r138_reask_female(_=None):
    try:
        return str(_bl131._FEMALE_EXACT.get(_r138_reask(), ""))
    except Exception:
        return ""


def _r138_prompt_clean(_=None):
    """prompt ทุกก้อนต้องไม่เหลือคำสั่งให้ขอไลน์แล้ว"""
    import faq_data as _fq
    for _mod in (_fq, _bl131):
        for _n in dir(_mod):
            if _n.startswith("__"):
                continue
            try:
                ...
                continue
            if isinstance(_v, str) and _R138_OLD_PROMPT in _v:
                return False
    return True


# ======================================================
# r139 — ยอดหนี้ถูกอ่านเป็นรายได้ (เคส FB-RS-20260903-516)
# ------------------------------------------------------
# Leads_RealtySmart แถว 517 · สัญญาณในแถวเขียนไว้ตรงๆ ว่า
#   "DSR ตอนนี้ 0% (ผ่อน 0 / รายได้ที่นับได้ 1,173,900) | ไม่มีภาระผ่อน
#    | วงเงินตอนนี้ 142.3M"   -> เกรด A
#
# ความจริงจากข้อความลูกค้า (ช่องรายได้เก็บไว้ทั้งก้อน)
#   "ตอนนี้มีหนี้รวมๆ 1,1739,000 ยังไม่ผิดนัดชำระ แต่เริ่มจ่ายไม่ไหว
#    พนักงานประจำ รายได้ 35,000"
#   = พนักงานประจำ รายได้ 35,000 · หนี้คงเหลือ ~1.7 ล้าน · เริ่มจ่ายไม่ไหว
#
# ต้นเหตุ: ข้อความมีทั้งคำว่าหนี้และคำว่ารายได้ในก้อนเดียว
#   _r100_reroute ทำงานเฉพาะตอน "มีคำฝั่งเดียว" -> ก้อนนี้เลยผ่านไปทั้งดุ้น
#   _parse_income หยิบเลขก้อนแรกที่เจอ = 1,173,900 (ยอดหนี้) มาเป็นเงินเดือน
#
# r139 แก้ 3 ชั้น
#   ก) ก้อนที่มีทั้งหนี้และรายได้ -> ตัดเอาเฉพาะบรรทัดที่มีคำว่ารายได้/เงินเดือน
#   ข) เพดานกันพลาด: อ่านรายได้ได้เกิน 500,000/เดือน จากข้อความที่พูดเรื่องหนี้
#      = แทบแน่ว่าเป็นยอดหนี้ -> ไม่เก็บ ติดธง ให้บอทถามใหม่
#   ค) คำว่า "จ่ายไม่ไหว/ผิดนัด/ค้างชำระ" = ติดธงแดงให้เซลเห็นก่อนโทร
#      (ยังไม่แตะการตีเกรด รอ Gift เคาะ)
# ======================================================
_R139_MAX_CHAT_INCOME = 500000
_R139_RISK_WORDS = ("จ่ายไม่ไหว", "ผ่อนไม่ไหว", "ชำระไม่ไหว", "ผิดนัด",
                    "ค้างชำระ", "ค้างจ่าย", "โดนทวง", "ปรับโครงสร้าง",
                    "จ่ายขั้นต่ำ", "หมุนไม่ทัน")


def _r139_income_clause(msg):
    """ข้อความมีทั้งหนี้และรายได้ -> คืนเฉพาะท่อนที่พูดเรื่องรายได้

    ไม่เข้าเงื่อนไข = คืนข้อความเดิมทั้งก้อน (ไม่แตะอะไรเลย)
    """
    s = str(msg or "")
    if not (_bl131._has_any(s, _bl131._DEBT_SAYS)
            and _bl131._has_any(s, _bl131._INCOME_SAYS)):
        return s
    parts = [p.strip() for p in re.split(r"[" + chr(10) + chr(13) + r"]+", s)
             if p.strip()]
    keep = [p for p in parts
            if _bl131._has_any(p, _bl131._INCOME_SAYS)
            and not _bl131._has_any(p, _bl131._DEBT_SAYS)]
    if not keep:
        return s
    cand = " ".join(keep).strip()
    try:
        if not _bl131._parse_income(cand):
            return s
    except Exception:
        return s
    return cand
...
def _r139_risk(msg):
    """ลูกค้าส่งสัญญาณว่าเริ่มจ่ายหนี้ไม่ไหว"""
    return _bl131._has_any(str(msg or ""), _R139_RISK_WORDS)


_R139_EXAM_TXT = ("ตอนนี้มีหนี้รวมๆ 1,1739,000 ยังไม่ผิดนัดชำระ แต่เริ่มจ่ายไม่ไหว"
                  + chr(10) + "พนักงานประจำ รายได้ 35,000")


def _r139_exam_pick(_=None):
    return _r139_income_clause(_R139_EXAM_TXT)


_R139_BASE_CAPTURE = CalmBotEngine._capture


def _capture_r139(self, state, field, msg):
    _raw = str(msg or "")
    try:
        if field == "income":
            _cut = _r139_income_clause(_raw)
            if _cut != _raw:
                print("[R139] ก้อนนี้มีทั้งหนี้และรายได้ — ตัดเอาเฉพาะท่อนรายได้: "
                      + _cut[:60])
                msg = _cut
    except Exception as e:
        print(f"[R139 ERROR cut] {e}")

    _out = _R139_BASE_CAPTURE(self, state, field, msg)
    ...
    try:
        ...
        if field == "income":
            _n = 0
            try:
                _n = int(_d.get("income_baht") or 0)
            except Exception:
                _n = 0
            if (_n > _R139_MAX_CHAT_INCOME
                    and _bl131._has_any(_raw, _bl131._DEBT_SAYS)):
                _d.pop("income_baht", None)
                _d.pop("income", None)
                _d.pop("income_total", None)
                self._add_signal(
                    state,
                    "⚠️ อ่านรายได้ได้ " + format(_n, ",")
                    + "/เดือน จากข้อความที่พูดเรื่องหนี้ — น่าจะเป็นยอดหนี้ "
                    + "ไม่ใช่เงินเดือน · ไม่เก็บ ให้ถามรายได้ใหม่ (r139)")
                print("[R139] รายได้ " + format(_n, ",")
                      + " มาจากประโยคเรื่องหนี้ — ไม่เก็บ ถามใหม่")
        if _r139_risk(_raw) and not state.get("r139_risk_flag"):
            state["r139_risk_flag"] = True
            self._add_signal(
                state,
                "🔴 ลูกค้าบอกเองว่าเริ่มจ่ายหนี้ไม่ไหว/ผิดนัด — "
                "ตรวจภาระจริงก่อนเสนอห้อง อย่าเพิ่งเชื่อเกรดอัตโนมัติ (r139)")
            print("[R139] ธงแดง — ลูกค้าบอกว่าจ่ายไม่ไหว")
    except Exception as e:
        print(f"[R139 ERROR post] {e}")
    return _out


CalmBotEngine._capture = _capture_r139


# ======================================================
# r140 — เคสธุรกิจ: อ่านช่วงรายได้ภาษาไทย + ถามให้ชัดว่ายอดขายหรือกำไร
# ------------------------------------------------------
# เคส FB-MA-20260903-771 (Leads_Millionaire แถว 772) เจ้าของธุรกิจ
#   ช่องรายได้ "รายได้แต่ละเดือนไม่รวมซื้อของเฉลี่ยประมาณ 4หมื่นถึง7-8 หมื่นค่ะ"
#   ตัวเลขที่ระบบใช้ 80,000 · ภาระ 25,000 · ไม่รู้อายุ · เกรด B
#
# (ก) ในระบบมีตัวจับ "รายได้เป็นช่วง" อยู่แล้ว เจอแล้วติดธงว่า
#     "บอกรายได้เป็นช่วง X–Y — ตีเกรดด้วยตัวสูง ต้องขอสลิปจริงยืนยันก่อนยื่น"
#     แต่ _income_range อ่านได้เฉพาะตัวเลขอารบิก (40000-80000)
#     ลูกค้าพิมพ์คำไทย "4หมื่นถึง7-8 หมื่น" -> ตัวจับไม่ทำงาน ไม่มีธงเตือนเลย
#     เซลเห็นแค่ 80,000 ไม่รู้ว่าเป็นปลายบนของช่วงที่ลูกค้าเดาเอง
#     แก้: อ่านหน่วยไทย (พัน/หมื่น/แสน/ล้าน) ได้ด้วย · ไม่แตะการตีเกรด
#
# (ข) Gift เคาะ 3 ก.ย. 2026: "เคสธุรกิจที่บอกรายได้กำกวม ให้ถามก่อน
#     ว่ายอดขายหรือกำไร" (กติกาเดิม 3 ก.ย. เช้า: ยอดขาย x margin 15% เข้า DSR)
#     เคสนี้ลูกค้าเขียน "ไม่รวมซื้อของ" ซึ่งอ่านได้ 2 ทาง
#       ยอดขายยังไม่หักของ -> ต้อง x15% = 12,000 -> เกรด D
#       หักของแล้ว = กำไรอยู่แล้ว -> ห้าม x15% ซ้ำ ไม่งั้นบี้เคสดีตาย
#     ถามครั้งเดียว โควตา 1 · ไม่ตอบชัด = ใช้ตามที่ลูกค้าบอก + ติดธงให้เซล
#     (ห้ามทิ้งเคส)
# ======================================================
_R140_MARGIN = 0.15
_R140_UNIT = (("ล้าน", 1000000), ("แสน", 100000), ("หมื่น", 10000), ("พัน", 1000))
_R140_TH_DIGIT = (("หนึ่ง", "1"), ("สอง", "2"), ("สาม", "3"), ("สี่", "4"),
                  ("ห้า", "5"), ("หก", "6"), ("เจ็ด", "7"), ("แปด", "8"),
                  ("เก้า", "9"), ("สิบ", "10"))
_R140_RANGE_WORDS = ("ถึง", "-", "–", "—", "~")

_R140_PAIR_RE = re.compile(
    "([0-9]+)[ ]*[-–—~][ ]*([0-9]+)[ ]*(ล้าน|แสน|หมื่น|พัน)")
_R140_ONE_RE = re.compile("([0-9]+(?:[.][0-9]+)?)[ ]*(ล้าน|แสน|หมื่น|พัน)")
_R140_PLAIN_RE = re.compile("(?<![0-9])([0-9]{4,7})(?![0-9])")


def _r140_amounts_th(msg):
    """ดึงจำนวนเงินทุกก้อนจากข้อความ รองรับหน่วยไทย

    "4หมื่นถึง7-8 หมื่น" -> [40000, 70000, 80000]
    """
    s = str(msg or "").replace(",", "")
    for _w, _d in _R140_TH_DIGIT:
        s = s.replace(_w, _d)
    out = []
    _unit = dict(_R140_UNIT)
    for m in _R140_PAIR_RE.finditer(s):
        _u = _unit.get(m.group(3), 1)
        out.append(int(float(m.group(1)) * _u))
        out.append(int(float(m.group(2)) * _u))
    _s2 = _R140_PAIR_RE.sub(" ", s)
    for m in _R140_ONE_RE.finditer(_s2):
        out.append(int(float(m.group(1)) * _unit.get(m.group(2), 1)))
    _s3 = _R140_ONE_RE.sub(" ", _s2)
    for m in _R140_PLAIN_RE.finditer(_s3):
        out.append(int(m.group(1)))
    return [n for n in out if 1000 <= n <= 100000000]
...
_bl131._income_range = _r140_income_range
...
# ---- (ข) ยอดขาย หรือ กำไร -------------------------------------------
_R140_BIZ_WORDS = ("ขายของ", "ค้าขาย", "ร้าน", "ธุรกิจ", "กิจการ", "แม่ค้า",
                   "พ่อค้า", "ขายออนไลน์", "ยอดขาย", "เจ้าของกิจการ", "ฟรีแลนซ์",
                   "รับจ้าง", "อาชีพอิสระ")
_R140_SALES_WORDS = ("ยอดขาย", "ยอดรวม", "ก่อนหัก", "ขายได้", "รายรับ",
                     "ยังไม่หัก", "ไม่ได้หัก", "รวมต้นทุน", "ยอดเข้าร้าน")
_R140_PROFIT_WORDS = ("กำไร", "หลังหัก", "สุทธิ", "หักแล้ว", "เหลือจริง",
                      "หักต้นทุนแล้ว", "หักค่าใช้จ่ายแล้ว", "เหลือเข้ากระเป๋า")
_R140_ASK = ("ขอเช็คนิดนึงครับ ยอดที่แจ้งมาคือ ยอดขายรวม "
             "หรือกำไรหลังหักต้นทุนแล้วครับ "
             "ธนาคารดูกำไรจริง ผมจะได้คิดวงเงินให้ตรงครับ")


def _r140_kind(msg):
    """คำตอบบอกว่าเป็นยอดขายหรือกำไร — อ่านไม่ออกคืนค่าว่าง"""
    s = str(msg or "")
    if _bl131._has_any(s, _R140_PROFIT_WORDS):
        return "profit"
    if _bl131._has_any(s, _R140_SALES_WORDS):
        return "sales"
    return ""


def _r140_is_biz(state):
    if (state or {}).get("self_employed"):
        return True
    _d = (state or {}).get("data") or {}
    return _bl131._has_any(str(_d.get("income") or ""), _R140_BIZ_WORDS)


_R140_BASE_DECIDE = CalmBotEngine._decide


def _decide_r140(self, msg, user_id, state, bucket, is_new):
    # ---- ก) กำลังรอคำตอบว่ายอดขายหรือกำไร ----------------------------
    try:
        _pend = state.pop("r140_pending", None)
        if _pend:
            _kind = _r140_kind(msg)
            _d = state.setdefault("data", {})
            if _kind == "sales":
                _new = int(int(_pend) * _R140_MARGIN)
                _d["income_baht"] = _new
                if _d.get("income_total"):
                    _d["income_total"] = _new
                self._add_signal(
                    state,
                    "ลูกค้ายืนยันว่าเป็นยอดขาย " + format(int(_pend), ",")
                    + "/เดือน — คิดรายได้จริงที่ margin "
                    + str(int(_R140_MARGIN * 100)) + "% = " + format(_new, ",")
                    + " (กติกา Gift 3 ก.ย. 69) · เซลขอ ภ.พ.30 ยืนยันตอนโทร")
                print("[R140] ยอดขาย " + str(_pend) + " x margin -> " + str(_new))
            elif _kind == "profit":
                self._add_signal(
                    state,
                    "ลูกค้ายืนยันว่า " + format(int(_pend), ",")
                    + "/เดือน คือกำไรหลังหักต้นทุนแล้ว — ใช้ตัวนี้เข้า DSR ได้")
                print("[R140] ยืนยันเป็นกำไร " + str(_pend))
            else:
                self._add_signal(
                    state,
                    "⚠️ เคสธุรกิจ — ถามแล้วว่ายอด " + format(int(_pend), ",")
                    + " เป็นยอดขายหรือกำไร ลูกค้ายังไม่ตอบชัด "
                    + "ใช้ตามที่แจ้งไปก่อน เซลถามตอนโทร (r140)")
                print("[R140] ตอบไม่ชัด — ใช้ตามที่แจ้ง ติดธงแทน")
    except Exception as e:
        print(f"[R140 ERROR resolve] {e}")

    out = _R140_BASE_DECIDE(self, msg, user_id, state, bucket, is_new)

    # ---- ข) เคสธุรกิจที่บอกรายได้กำกวม -> ถาม 1 ครั้ง ------------------
    try:
        bubbles, grade = out
        _d = state.get("data") or {}
        _n = 0
        try:
            ...
            _n = 0
        if (bubbles and _n and not state.get("r140_asked")
                and not state.get("done") and _r140_is_biz(state)
                and not _r140_kind(str(_d.get("income") or ""))):
            state["r140_asked"] = True
            state["r140_pending"] = _n
            print("[R140] เคสธุรกิจ รายได้ " + format(_n, ",")
                  + " ยังไม่ชัดว่ายอดขายหรือกำไร — ถาม 1 ครั้ง")
            return list(bubbles) + [_R140_ASK], grade
    except Exception as e:
        print(f"[R140 ERROR ask] {e}")
    return out
