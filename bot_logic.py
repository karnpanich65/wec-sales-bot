...
# ======================================================
# r68 (Gift 21 ส.ค. 2026) — "ไม่เอาแล้วสวิตช์ปิดนี่ เอาออกให้หมด"
# ------------------------------------------------------
# ถอด BOT_MUTED_PAGES / page_muted ออกทั้งชุด ไม่มีสวิตช์ "ปิดบอททั้งเพจ"
# แบบเงียบ ๆ อีกแล้ว เพราะรอบ 21 ส.ค. มันทำให้เพจ Millionaire Asset
# เงียบยกเพจ โดยหน้า Railway ไม่มีตัวแปรให้เห็นเลยว่าโดนปิดอยู่
# (ค่าถูก hard-code เป็น default ในไฟล์นี้ — หาไม่เจอ ปิดไม่ได้ ตามไม่ทัน)
#
# เหลือสวิตช์ 2 ตัวเท่านั้น:
#   BOT_PAUSE_PAGES (บรรทัด ~1231) = ฉุกเฉินระดับเพจ ตั้งจาก Railway ได้เลย
#       หยุดสนิท เขียนแค่ log · ไม่เก็บลีด ไม่แจกเคส ไม่เข้ารีพอร์ต
#       ข้อจำกัด: ยังไม่กันคอมเมนต์ / private reply (คนละไฟล์ อยู่ main.py)
#   BOT_KILL_PHRASES (r64) = ปิดรายแชท ตัวที่ใช้จริงในงานประจำวัน
#       เซลพิมพ์ "ขอบคุณที่สนใจครับบ" = ปิดแชทนั้น · "#เปิดบอท" = เปิดคืน
#
# ต้นเหตุที่เคยต้องปิดทั้งเพจ (บอทพิมพ์ทับพี่ไปป์ 21 ส.ค. 12:38) แก้ที่
# SALES_BY_PAGE ตั้งแต่ r52 แล้ว — ชื่อ "ไปป์" อยู่ในรายชื่อเซลเพจ MA
# ======================================================
...
class BotEngine:
    ...
    def process(self, user_message: str, user_id: str,
        ...
        # r68 — ล้างร่องรอย "ปิดบอทรายเพจ" ที่ค้างมาจากยุค r60-r67
        # ตอนเพจถูกมิวต์ ทุกแชทของเพจนั้นถูกยัดเข้า handover แบบไม่หมดอายุ
        # ถ้าไม่ล้างทิ้ง แชทเหล่านั้นจะยังเงียบต่อไปจนครบ TTL ทั้งที่ถอดมิวต์แล้ว
        if state.get("handover_by") == "(ปิดบอทรายเพจ)":
            state["handover"] = False
            state["handover_by"] = ""
            state["handover_ended_at"] = _now()
            print(f"[UNMUTE] {user_id[:8]}... ล้าง handover ค้างจากการปิดบอทรายเพจ")
